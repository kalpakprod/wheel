...
2. GitHub Search API: `gh search repos "<ключевые слова>" --sort stars --limit 20`, плюс поиск по topics из `capabilities.yaml`.
3. `awesome-*` списки по теме — они уже курированы людьми.
4. Обычный веб-поиск — для платных продуктов и того, что не живёт на GitHub.
...
**Запись решения — по критерию ADR** (`mattpocock/skills` → `domain-modeling`). Все три условия, иначе не пишем:
