...
2. Каталог проектов: `~/.claude/wheel/catalog.local.jsonl` (свой, если пользователь собирает его сам), затем `~/.claude/wheel/catalog.jsonl` (общий, обновляется фоном раз в сутки). По строке JSON на проект, только `grep`, файл не читается целиком.
   ...
   Оценки «стоит ли ставить» в общем каталоге нет намеренно: она зависит от того, что уже есть в стеке у конкретного человека. Меряй зазор сам, на шаге `S4`. В своём каталоге могут быть дополнительные поля — `tier`, `useful`, `verdict`: это прошлые решения самого пользователя, им доверяй.
