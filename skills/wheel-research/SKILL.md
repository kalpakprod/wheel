...
- OpenCLI only for a page that requires an existing authenticated user browser session. DonSeTch does not replace login-backed platform adapters or platform-specific APIs.
- Never drive Reddit through a logged-in browser session, including `opencli reddit <command>`. Reddit's Responsible Builder Policy forbids masking automated access and mixing an app with a human account, and its enforcement suspends the account, not the tool. Reddit evidence comes from app-only OAuth or the archive lane, and from nowhere else. The write verbs of any Reddit adapter (`upvote`, `comment`, `reply`, `subscribe`) are out of scope for research at all times.
